...
void LoopClosing::Run() {
  ...
  const long long period_ns = 50000000;   // 50 ms
  ...
  size_t prio_index = 0;
  sch_params.sched_priority = EDF_ACTIVE_TABLE[LOOP_CLOSING_THREAD][prio_index];
...
#ifdef SCHED_EDF_VDSD
        prio_index = (prio_index + 1) % EDF_ACTIVE_TABLE[LOOP_CLOSING_THREAD].size();
        if (pthread_setschedprio(pthread_self(),
                                 EDF_ACTIVE_TABLE[LOOP_CLOSING_THREAD][prio_index])) {
